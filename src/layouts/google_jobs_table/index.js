...
// Data
import dataFun from "layouts/google_jobs_table/data/authorsTableData";
import projectsTableData from "layouts/google_jobs_table/data/projectsTableData";
import axios from "axios";
import Loader from "../../examples/loader/loader";

function google_jobs_table() {
  ...
  const sendBtn = () => {
    (async () => {
      ...
      try {
        const response = await axios.get(`${baseURL}/scraper/google_jobs/send`, {
          headers: {
  ...
  const deleteBtn = () => {
    (async () => {
      setIsLoading(dispatch, true);
      try {
        const response = await axios.get(`${baseURL}/scraper/google_jobs/clean`, {
          headers: {
            "Content-Type": "application/json",
          },
        });
  ...
  const gettingStatus = async () => {
    try {
      const response = await axios.get(`${baseURL}/scraper/google_jobs/status`, {
        headers: {
          "Content-Type": "application/json",
        },
      });
  ...
  const startBtn = async () => {
    ...
    if (statusBtn.available) {
      try {
        ...
        const response = await axios.get(`${baseURL}/scraper/google_jobs/start`, {
          headers: {
            "Content-Type": "application/json",
          },
        });
  ...
  const stopBtn = async () => {
    ...
    if (statusBtn.available) {
      try {
        ...
        const response = await axios.get(`${baseURL}/scraper/google_jobs/stop`, {
          headers: {
            "Content-Type": "application/json",
          },
        });
            ...
            {rows ? (
              <Card>
                <SoftBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
                  <SoftTypography variant="h6">Google Jobs table</SoftTypography>
                </SoftBox>
                {statusBtn.available ? (
                  <SoftBox
                    display="flex"
                ...
                >
                  {!statusBtn?.available ? (
                    ...
                    <SoftTypography variant="h7" color="success">
                      Available
                    </SoftTypography>
                  )}
                </SoftBox>
...
google_jobs_table.propTypes = {
  paginationData: PropTypes.object.isRequired,
};

export default google_jobs_table;
